...
// MARK: - Librarian LIGHT panel — Cucumber Water register
//
// T's decision 2026-07-24: the Librarian is NO LONGER dark-only. It gets a
// LIGHT mode — a distinct EMOTIONAL register ("between cream and glass"), NOT a
// lightened Solar Flare. Dark stays the elaborate SolarFlareMaterial above,
// byte-identical. The two registers are selected by ONE typed style
// (`LibrarianPanelMaterial` below), the same pattern PeekPillStyle established —
...
struct LibrarianPanelMaterial: View {
    ...
    var body: some View {
        if colorScheme == .dark {
            SolarFlareMaterial(isDragging: isDragging, activeAccent: activeAccent)
        } else {
            LibrarianLightMaterial()
        }
    }
}
