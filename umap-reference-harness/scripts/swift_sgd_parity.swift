#!/usr/bin/env swift
// SB139 Stage 4a step 4.3 — host-side parity check for umappp::optimize_layout.
//
// Mirrors AirPad/Services/UMAP/UMAPSGD.swift end-to-end. Loads the fit
// fixture (rngSeed, training points), the intermediates dump (fuzzy
// simplicial set), and the final umappp output (golden 2D coords). Then
// runs the full Swift SGD pipeline (random init → similarities_to_epochs
// → optimize_layout) and diffs Swift's final coords against the harness's.
//
// Reports:
//  - maxAbsCoordErr (the precision floor for downstream stages)
//  - bit-exact rate (how many coords match exactly)
//  - drift distribution (median, p90, p99 absolute error)
//  - whether final coords fall within the 1e-6 ceiling
//
...
//   swift scripts/swift_sgd_parity.swift \
...
// ===========================================================================
// MARK: - SGD (copy of UMAPSGD.swift)
// ===========================================================================
...
// find_ab — copy of UMAPFindAB.swift's umapFindAB. Bit-exact against
// umappp::find_ab; verified by swift_find_ab_parity.swift (step 4.1).
...
guard args.count >= 4 else {
    FileHandle.standardError.write(
        "usage: swift_sgd_parity.swift <fit_fixture.json> <intermediates.json> <umappp_result.json>\n"
            .data(using: .utf8)!)
    exit(2)
}
...
print("SGD parity report — synth_50x4 (n=\(nobs), numDim=\(numDim), nEpochs=\(numEpochs))")
print("  a=\(a) b=\(b)")
...
if maxAbsErr <= ceilingTolerance {
    ...
    // Print first 10 worst offenders.
